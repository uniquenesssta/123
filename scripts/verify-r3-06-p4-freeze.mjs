...
const orchestrationFacade = read("crates/application/src/services/p4_orchestration/facade.rs");
...
check(service.includes("execute_p4_freeze::execute") && orchestrationUseCase.includes(".execute_p4_freeze_task("), "Prediction Service/P4 orchestration 未接通 freeze use case");
check(!facade.includes("fn execute_p4_freeze_task"), "ApplicationService facade 仍保留仅供 orchestration 的私有 freeze helper");
check(!fs.existsSync(path.join(root, "crates/application/src/p4_orchestration.rs")), "旧 p4_orchestration owner 必须删除");
check(orchestrationFacade.includes("process_next_p4_orchestration_job") && orchestrationUseCase.includes("P4_FREEZE_JOB"), "P4 orchestration facade/use case 未接通");
check(orchestrationUseCase.includes("execute_p4_freeze_task") && orchestrationUseCase.includes("execute_p4_research_task"), "P4 worker 未保持 Research/Prediction 双职责委托");
check(!orchestrationUseCase.includes("execute_p4_openai_research"), "P4 orchestration 不得接管 Research 联网执行实现");
