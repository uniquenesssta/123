...
quotepath_failed = os.environ["R4_03_QUOTEPATH_FAILED_RUN"]
...
new = old + f" run `{env_failed}` 随后通过 R4-03 最小门禁与完整 architecture，但因临时 runner 未先执行既有 `npm run setup`，frontend 在缺少已声明的 `typescript` 开发依赖时停止；workspace Clippy/tests 未执行且仍未提交生产源码。run `{publish_failed}` 已通过全部质量、冻结范围、文档与 transient cleanup 门禁，最终仅因 npm cache 临时落在仓库根而被 clean-worktree 发布门禁拒绝，仍未提交生产源码。run `{cleanup_failed}` 再次通过全部质量、范围与文档门禁，但 cleanup 在精确暂存前检查 untracked，误将本任务应新增文件视为非法项并停止，仍未提交生产源码。run `{quotepath_failed}` 通过全部质量、范围、文档、精确暂存与 untracked 清理门禁，最终仅因 Git 默认 quotePath 将中文记录文件名八进制转义而误判 staged allowlist，仍未提交生产源码。"
...
new = old + f"第三次 run `{env_failed}` 已通过最小门禁与 architecture，但 frontend 因临时 runner 未先准备既有 `typescript` 开发依赖而停止，Clippy/workspace tests 按 fail-fast 未执行；第四次 run `{publish_failed}` 已通过完整质量与范围门禁，仅因 npm cache 临时落在仓库根而被严格 clean-worktree 发布门禁拒绝；第五次 run `{cleanup_failed}` 再次通过完整质量与范围门禁，但 cleanup 在精确暂存前检查 untracked，误将任务应新增文件本身拒绝；第六次 run `{quotepath_failed}` 已通过完整质量、范围、精确暂存与零额外 untracked 检查，仅因 Git 默认 quotePath 将中文任务记录路径转义而误判最终 allowlist；"
...
print(
    ...
    f"cleanup-order failure={cleanup_failed}, quotePath failure={quotepath_failed}"
)
