...
new = old + f" run `{env_failed}` 随后通过 R4-03 最小门禁与完整 architecture，但因临时 runner 未先执行既有 `npm run setup`，frontend 在缺少已声明的 `typescript` 开发依赖时停止；workspace Clippy/tests 未执行且仍未提交生产源码。run `{publish_failed}` 已通过全部质量、冻结范围、文档与 transient cleanup 门禁，最终仅因 npm cache 临时落在仓库根而被 clean-worktree 发布门禁拒绝，仍未提交生产源码。"
...
new = old + f"第三次 run `{env_failed}` 已通过最小门禁与 architecture，但 frontend 因临时 runner 未先准备既有 `typescript` 开发依赖而停止，Clippy/workspace tests 按 fail-fast 未执行；第四次 run `{publish_failed}` 已通过完整质量与范围门禁，仅因 npm cache 临时落在仓库根而被严格 clean-worktree 发布门禁拒绝；"
...
print(
    ...
    f"dependency-setup failure={env_failed}, publication-cleanup failure={publish_failed}"
)
