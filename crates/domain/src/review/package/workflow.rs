...
fn all_workflow_actions() -> [MatchReviewPackageWorkflowAction; 8] {
    use MatchReviewPackageWorkflowAction as A;
    [
        A::ExportPackage,
        A::PreviewImport,
        A::ConfirmImport,
...
fn allowed_actions(
    ...
    use MatchReviewPackageWorkflowAction as A;
    use MatchReviewPackageWorkflowStatus as S;
    let mut actions = vec![A::ExportPackage];
    match status {
        S::Exported | S::PreviewBlocked => actions.push(A::PreviewImport),
        S::PreviewValid => {
            actions.push(A::PreviewImport);
            actions.push(A::ConfirmImport)
        }
        S::Confirmed => actions.push(A::CommitFacts),
        S::FactsCommitted => actions.push(A::GenerateReview),
        S::ReviewCreated => {
            actions.push(A::InspectSettlementReadiness);
            actions.push(A::SettleReview)
        }
        S::Settled => actions.push(A::OpenAnalytics),
        S::Superseded => actions.clear(),
    }
    actions
}
fn completed_steps(
    ...
    use MatchReviewPackageWorkflowStatus as S;
    use MatchReviewPackageWorkflowStep as T;
    if status == S::Superseded {
        return Vec::new();
    }
    let mut steps = vec![T::ExportPackage];
    if previewed {
        steps.push(T::CompleteExternalData)
    }
    if matches!(
        status,
        S::PreviewValid | S::Confirmed | S::FactsCommitted | S::ReviewCreated | S::Settled
    ) {
        steps.push(T::PreviewImport)
    }
    if matches!(
        status,
        S::Confirmed | S::FactsCommitted | S::ReviewCreated | S::Settled
    ) {
        steps.push(T::ConfirmImport)
    }
    if matches!(status, S::FactsCommitted | S::ReviewCreated | S::Settled) {
        steps.push(T::CommitFacts)
    }
    if matches!(status, S::ReviewCreated | S::Settled) {
        steps.push(T::GenerateReview)
    }
    if status == S::Settled {
        steps.push(T::SettleReview)
    }
    steps
}
fn next_action(
    ...
    use MatchReviewPackageWorkflowStatus as S;
    match status {
        S::Exported | S::PreviewBlocked => Some(A::PreviewImport),
        S::PreviewValid => Some(A::ConfirmImport),
        S::Confirmed => Some(A::CommitFacts),
        S::FactsCommitted => Some(A::GenerateReview),
        S::ReviewCreated => Some(A::InspectSettlementReadiness),
        S::Settled => Some(A::OpenAnalytics),
        S::Superseded => None,
    }
}
fn blocking_reason(
    ...
    match action {
        A::ExportPackage => "当前资料包不能重新导出",
        A::PreviewImport => match status {
            S::Confirmed | S::FactsCommitted | S::ReviewCreated | S::Settled => {
                "资料包已人工确认，不能覆盖本轮预检结果"
            }
            _ => "尚未导出可供预检的资料包",
        },
        A::ConfirmImport => match status {
            S::Exported => "尚未导入并预检填写后的资料包",
            S::PreviewBlocked => "预检仍有阻断错误",
            S::Confirmed | S::FactsCommitted | S::ReviewCreated | S::Settled => {
                "资料包已经人工确认"
            }
            _ => "资料包尚未通过预检",
        },
        A::CommitFacts => match status {
            S::Exported | S::PreviewBlocked | S::PreviewValid => "资料包尚未人工确认",
            S::FactsCommitted | S::ReviewCreated | S::Settled => "真实赛后事实已经写入",
            _ => "当前资料包不能写入赛后事实",
        },
        A::GenerateReview => match status {
            S::Exported | S::PreviewBlocked | S::PreviewValid | S::Confirmed => {
                "真实赛后事实尚未写入"
            }
            S::ReviewCreated | S::Settled => "正式复盘已经生成",
            _ => "当前资料包不能生成正式复盘",
        },
        A::InspectSettlementReadiness | A::SettleReview => match status {
            S::Settled => "正式结算已经完成",
            _ => "正式复盘尚未生成",
        },
        A::OpenAnalytics => "正式结算尚未完成",
    }
}
...
mod workflow_tests {
    ...
    fn workflow_state_exposes_one_authoritative_next_action() {
        ...
        for (status, expected) in [
            (S::Exported, Some(A::PreviewImport)),
            (S::PreviewBlocked, Some(A::PreviewImport)),
            (S::PreviewValid, Some(A::ConfirmImport)),
            (S::Confirmed, Some(A::CommitFacts)),
            (S::FactsCommitted, Some(A::GenerateReview)),
            (S::ReviewCreated, Some(A::InspectSettlementReadiness)),
    ...
    fn blocked_preview_does_not_unlock_confirmation() {
        use MatchReviewPackageWorkflowAction as A;
        use MatchReviewPackageWorkflowStatus as S;
        let actions = allowed_actions(S::PreviewBlocked);
        assert!(actions.contains(&A::PreviewImport));
        assert!(!actions.contains(&A::ConfirmImport));
        assert_eq!(
            blocking_reason(S::PreviewBlocked, A::ConfirmImport),
            "预检仍有阻断错误"
        )
    }
    #[test]
    fn settled_workflow_only_opens_analysis_or_starts_new_export() {
        use MatchReviewPackageWorkflowAction as A;
        use MatchReviewPackageWorkflowStatus as S;
        let actions = allowed_actions(S::Settled);
        assert_eq!(actions, vec![A::ExportPackage, A::OpenAnalytics]);
        assert!(completed_steps(S::Settled, true)
    ...
    fn persisted_status_round_trips_without_stringly_typed_logic() {
        ...
        for status in [
            ...
            assert_eq!(S::parse(status.as_str()), Ok(status));
            assert_eq!(
