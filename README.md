...
- R4-01 已将 Persistence Store/Error/Pool/Migration/Health/Statistics 从旧 `lib.rs`/`connection.rs` 拆入职责目录；历史 0001–0046 migration SQL、公共 Persistence API、DatabaseOptions Serde、错误语义与生产依赖保持不变。严格 hard gate run `31615481637` 在提交前通过专项、数据库基线、architecture、persistence crate check/tests/Clippy；18 个专用 PostgreSQL 集成测试保持 ignored，未执行 destructive reset。状态为 `VERIFYING`。
